...
def run_single_evaluation(args: Tuple) -> Tuple[AccuracyMetrics, Optional[str]]:
    ...
    test_content, train_content, config_dir, work_dir, species_name, min_intron_len = (
        args
    )
    ...
    try:
        ...
        subprocess.run(
            etraining_cmd, capture_output=True, check=True, text=True, timeout=300
        )
    ...
    try:
        ...
        result = subprocess.run(
            augustus_cmd, capture_output=True, check=True, text=True, timeout=600
        )
...
class AugustusOptimizer:
    ...
    def setup(self):
        ...
        logger.info(
            f"Optimizer setup: {len(self.test_genes)} test, {len(self.train_genes)} train genes for CV"
        )
            ...
            with open(self.metapars_file) as f:
                for line in f:
                    ...
                    if int_match:
                        ...
                        low, high = float(float_match.group(1)), float(
                            float_match.group(2)
                        )
    ...
    def _load_gene_models(self):
        ...
        n_test = max(
            min(n_test, self.max_test_genes), min(n_genes, self.min_test_genes)
        )
        ...
        for record in content.split("//\n"):
            ...
            if join_match:
                for span in join_match.group(1).split(","):
                    ...
                    if coord_match:
                        cds_length += (
                            abs(int(coord_match.group(2)) - int(coord_match.group(1)))
                            + 1
                        )

            genes.append(
                GeneModelGB(
                    content=record + "//\n", gene_name=gene_name, cds_length=cds_length
                )
            )
    ...
    def evaluate_params(self, params: Dict[str, float]) -> float:
        ...
        try:
            ...
            for i, test_batch in enumerate(batches):
                ...
                other_batches_content = "".join(
                    batch_contents[j] for j in range(len(batches)) if j != i
                )
                train_content = (
                    train_genes_content + other_batches_content + onlytrain_content
                )
            ...
            with ProcessPoolExecutor(max_workers=n_workers) as executor:
                futures = [
                    executor.submit(run_single_evaluation, args) for args in eval_args
                ]
                for future in as_completed(futures):
                    try:
                        ...
                        if error_str:
                            logger.warning(
                                f"A batch failed during optimization: {error_str}"
                            )
                        if metrics.weighted_accuracy > 0:
                            all_metrics.append(metrics)
                    except Exception as e:
                        logger.warning(
                            f"Batch evaluation future failed: {e}", exc_info=True
                        )
    ...
    def optimize(self) -> Dict[str, float]:
        ...
        study.optimize(
            objective, n_trials=self.n_trials, n_jobs=1, show_progress_bar=True
        )
...
def main():
    ...
    parser.add_argument(
        "--n-trials", type=int, default=100, help="Optimization trials (default: 100)"
    )
    parser.add_argument(
        "--min-intron-len", type=int, default=30, help="Min intron length (default: 30)"
    )
    parser.add_argument(
        "--use-memory", action="store_true", help="Use /dev/shm for temp files"
    )
    parser.add_argument(
        "--output-dir", default="optimize_output", help="Output directory"
    )
