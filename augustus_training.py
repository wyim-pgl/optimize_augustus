...
try:
    ...
    print(
        "Warning: BioPython not installed. Install with: pip install biopython",
        file=sys.stderr,
    )
...
class FastaReader:
    ...
    def __init__(self, fasta_path: Path):
        ...
        self.index: Dict[str, Tuple[int, int]] = (
            {}
        )  # seqid -> (length, header_line_number)
            ...
            for line_num, line in enumerate(f):
                if line.startswith(">"):
                    ...
                    self.index[seqid] = (
                        0,
                        line_num,
                    )  # Will be updated when sequence ends
    ...
    def get_sequence(self, seqid: str, start: int = None, end: int = None) -> str:
        ...
        if start < 1 or end > seq_len or start > end:
            raise ValueError(
                f"Invalid coordinates: {start}-{end} for sequence of length {seq_len}"
            )
...
class GenBankWriter:
    ...
    def gene_to_genbank(self, gene: GeneModel) -> str:
        ...
        cds_feature = SeqFeature(
            cds_location, type="CDS", qualifiers={"gene": [gene.gene_id]}
        )
    ...
    def _gene_to_genbank_fallback(self, gene: GeneModel) -> str:
        ...
        if gene.strand == "-":
            gb_lines.append(
                f"     gene            complement({gene_start}..{gene_end})"
            )
        else:
...
class AugustusTrainer:
    ...
    def __init__(
        ...
        self.start_codons = [
            c.strip().upper() for c in start_codons.split(",") if c.strip()
        ]
        ...
        self.config_path_orig = Path(
            os.environ.get("AUGUSTUS_CONFIG_PATH", self.config_path)
        )
    ...
    def setup(self):
        ...
        self.genes = GFF3Parser.parse(
            self.gff3_file, include_all_isoforms=self.include_all_isoforms
        )
        ...
        if self.onlytrain_gff3:
            ...
            self.onlytrain_genes = GFF3Parser.parse(
                self.onlytrain_gff3, include_all_isoforms=self.include_all_isoforms
            )
            # Remove genes that are in main training set
            main_ids = {g.gene_id for g in self.genes}
            self.onlytrain_genes = [
                g for g in self.onlytrain_genes if g.gene_id not in main_ids
            ]
            self.logger.info(
                f"Found {len(self.onlytrain_genes)} additional onlytrain genes"
            )
    ...
    def step1_prepare_config(self):
        ...
        if self.augustus_species_start_from:
            src_params = (
                self.config_path_orig
                / "species"
                / self.augustus_species_start_from
                / f"{self.augustus_species_start_from}_parameters.cfg"
            )
            ...
            if src_params.exists():
                ...
                content = content.replace(
                    self.augustus_species_start_from, self.species_name
                )
                with open(dst_params, "w") as f:
                    f.write(content)
                self.logger.info(
                    f"Copied parameters from {self.augustus_species_start_from}"
                )
    ...
    def step2_convert_to_genbank(self):
        ...
        self.logger.info(
            f"Converting {total_genes} total genes to GenBank format using {self.cpu} workers..."
        )
        ...
        try:
            with ProcessPoolExecutor(
                ...
                for batch_start in range(0, total_genes, batch_size):
                    ...
                    futures = {
                        executor.submit(_run_gb_conversion, gene): gene.gene_id
                    ...
                    for future in as_completed(futures):
                        ...
                        try:
                            ...
                            self.logger.warning(
                                f"Failed to convert gene {gene_id}: {e}"
                            )

                        processed_count += 1
                        if processed_count % 100 == 0 or processed_count == total_genes:
                            self.logger.info(
                                f"Converted {processed_count}/{total_genes} genes..."
                            )
        finally:
            # Close output files
    ...
    def step3_filter_bad_genes(self):
        ...
        try:
            ...
            if result.returncode != 0:
                self.logger.error(
                    f"Failed to create temporary species for validation: {result.stderr}"
                )
                raise RuntimeError(
            ...
            result = self._run_subprocess_with_file_redirect(
                cmd, stderr_file=etraining_err
            )
            if result.returncode != 0:
                self.logger.error(
                    f"etraining failed with return code {result.returncode}"
                )
                raise RuntimeError(
    ...
    def step4_split_train_test(self):
        ...
        if n_genes >= self.min_gene_number:
            ...
            self.logger.info(
                f"Split: {len(self.train_genes)} training, {len(self.test_genes)} testing"
            )
        else:
            ...
            self.logger.warning(
                f"Insufficient genes ({n_genes} < {self.min_gene_number})"
            )
            self.logger.warning(
                "Using all genes for both training and testing (unreliable accuracy)"
            )
    ...
    def step5_first_training(self) -> AccuracyMetrics:
        ...
        if result.returncode != 0:
            self.logger.error(
                f"augustus accuracy test failed with return code {result.returncode}"
            )
            raise RuntimeError(
    ...
    def _update_stop_codon_freqs(self, output_source: str):
        ...
        if len(freqs) == 3:
            params_file = (
                ...
                / "species"
                / self.species_name
                / f"{self.species_name}_parameters.cfg"
            )
            ...
            content = re.sub(
                r"(/Constant/amberprob\s+)\S+", rf'\g<1>{freqs["tag"]}', content
            )
            content = re.sub(
                r"(/Constant/ochreprob\s+)\S+", rf'\g<1>{freqs["taa"]}', content
            )
            content = re.sub(
                r"(/Constant/opalprob\s+)\S+", rf'\g<1>{freqs["tga"]}', content
            )
            ...
            self.logger.info(
                f"Updated stop codon freqs: TAG={freqs['tag']:.4f}, TAA={freqs['taa']:.4f}, TGA={freqs['tga']:.4f}"
            )
    ...
    def _run_subprocess_with_file_redirect(
        self,
        cmd: List[str],
        ...
        try:
            result = subprocess.run(
                cmd, stdout=stdout_handle, stderr=stderr_handle, text=True
            )
            return result
        finally:
            if stdout_file:
    ...
    def _run_perl_optimization(self, train_file: Path, onlytrain_file: Path):
        ...
        if result.returncode != 0:
            self.logger.warning(
                f"optimize_augustus.pl finished with return code {result.returncode}"
            )
    ...
    def step7_second_training(self) -> AccuracyMetrics:
        ...
        # Redirect stdout directly to file instead of capturing in memory
        result = self._run_subprocess_with_file_redirect(
            cmd, stdout_file=etraining_out2
        )
        if result.returncode != 0:
            self.logger.error(f"etraining failed with return code {result.returncode}")
            raise RuntimeError(
        ...
        # Redirect stdout directly to file instead of capturing in memory
        result = self._run_subprocess_with_file_redirect(
            cmd, stdout_file=secondtest_out
        )
        if result.returncode != 0:
            self.logger.error(
                f"augustus accuracy test failed with return code {result.returncode}"
    ...
    def _set_start_codons(self):
        ...
        exon_probs_file = (
            ...
            / "species"
            / self.species_name
            / f"{self.species_name}_exon_probs.pbl"
        )

        if not exon_probs_file.exists() or not self.start_codons:
            self.logger.warning(
                f"Cannot set start codons: {exon_probs_file} not found or no codons specified."
            )
            return
        ...
        content = re.sub(
            r"\[STARTCODONS\].*?\[LENGTH\]", new_section, content, flags=re.DOTALL
        )
        ...
        self.logger.info(
            f"Set start codons to: {', '.join(self.start_codons)} with uniform probability."
        )

    def step8_compare_and_finalize(
        self, first_metrics: AccuracyMetrics, second_metrics: AccuracyMetrics
    ):
...
Examples:
    ...
    parser.add_argument(
        "-o",
        "--output-dir",
        default="augustus_training_output",
    ...
    parser.add_argument(
        "--augustus-species-start-from", help="Start from existing species parameters"
    )
    parser.add_argument(
        "--onlytrain-gff3",
    ...
    parser.add_argument(
        "--cpu", type=int, default=8, help="Number of CPUs (default: 8)"
    )
    parser.add_argument(
        "--n-trials",
        type=int,
    ...
    parser.add_argument(
        "--use-memory", action="store_true", help="Use /dev/shm for temp files (faster)"
    )
    parser.add_argument(
        "--optimize-method",
        type=int,
